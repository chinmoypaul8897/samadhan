...
## Current State
- **Phase:** Depth · **Chunk:** **C7 — Track — MACHINERY COMPLETE ✅, push gate pending 🔑** (status state-machine + FCM + live timeline). Code built + **build clean** + **local machinery 21/21** (transition graph, role-gate, illegal-move 409, status_change activity, resolvedAt/verifiedAt, reopen-clear, notify non-blocking). **Blocked only on the founder generating the Web Push (VAPID) public key** (Firebase Console → Project Settings → Cloud Messaging → Web Push certificates → Generate key pair — console-only, not API-creatable). Once pasted: bake `NEXT_PUBLIC_VAPID_PUBLIC_KEY` → build `:c7` → deploy → headed push-receipt check (web push can't be verified headless). 6 atomic commits pushed; **not yet deployed** (live still serves `:c6`).
...
### C7 — Track: status machine + FCM + timeline — MACHINERY COMPLETE ✅ (local 21/21), push gate pending 🔑 — opens Phase: Depth
...
**PENDING (the only gate left): the Web Push (VAPID) public key** — founder generates it in the Firebase Console (Project Settings → Cloud Messaging → Web Push certificates → Generate key pair; **provably console-only** — neither the Firebase Management nor FCM REST API exposes web-push certificates). On receipt: bake `NEXT_PUBLIC_VAPID_PUBLIC_KEY` (build-arg already wired in cloudbuild + Dockerfile) → build `:c7` → deploy → **headed** push-receipt check (web push can't be verified in headless Chromium — no push service). Not yet deployed (live serves `:c6`).
