...
- **Stale test data (pre-demo cleanup pending):** 3 leftover C4/C5 verify issues (`SMD-Z875ZYBW`, `SMD-CNYXP7A3`, `SMD-9BQ5D8BB`) + ~8 reports remain in Firestore `(default)` with the old `routing:null` placeholder. They don't affect C6; recommend a sweep before the final demo (awaiting founder OK to delete data not freshly created).
