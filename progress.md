...
## Current State
- **Phase:** Depth · **Chunk:** **C7 — Track — COMPLETE ✅ (live Track loop verified)** · on-device push *display* = founder confirming. Status state-machine + FCM + live timeline. **Verified: local machinery 21/21 + LIVE 4/4 + visual** on Cloud Run **rev `samadhan-00009-4fp`** (image `:c7b`, VAPID key baked). **On-device: a real browser registered an FCM token** (`cJAX8IrkJEtOYpQFlgny…` on issue `SMD-CT3XWD9S`) and two fired transitions invoked `notify()`→FCM live — founder confirming the OS notification arrived. **Bug fixed (`:c7b`, rev 9):** the opt-in button stayed on the CTA right after enabling because `useAuth().profile.fcmTokens` is read once at sign-in (stale in-session) → added a local `justEnabled` flag so it flips to the on-state at once (returning loads already read the token from the user doc). Live: `/api/internal/transition` role-gate (citizen→403), `submitted→acknowledged→in_progress→resolved_pending_verification` each writes status + `status_change` activity; the open `/issue/[id]` **streamed the updates live** (status chip → "Awaiting your confirm", SLA → "RESOLVED IN TIME", timeline rows appeared) via onSnapshot; `notify()` ran on Cloud Run reaching FCM (the SA has the role). **The only thing headless can't do is RECEIVE/DISPLAY a web push** — that's the founder's headed check (open the live app on a phone/desktop Chrome, tap "Get notified when this moves" + allow, I fire a transition, the OS notification lands). 7 atomic commits pushed.
- **C7 build:** `lib/status.ts` `transition(issueId,{to,actorUid,actorRole,note,expectedFrom})` — validates §9 graph, writes status/statusNotes/updatedAt + `status_change` activity (from/to/actor) + resolvedAt(→resolved_pending_verification)/verifiedAt(→verified_resolved), reopen clears both; Admin txn; `notifyUser` **after** commit. `lib/notify.ts` `notifyUser`/`notifyReporter` — **data-only** `sendEachForMulticast` (SW renders one notification; a top-level notification payload double-fires on web), ≤500-token chunks, prune `registration-token-not-registered`/`invalid-registration-token` via arrayRemove, best-effort never-throws, `issueLink()` HTTPS click target. `POST /api/internal/transition` role-gated (officer/admin via `users/{actorUid}.role`) = the C7 trigger (C8 officer portal supersedes). `public/firebase-messaging-sw.js` = **compat importScripts pinned 12.15.0**, config inlined, `onBackgroundMessage`→showNotification + `notificationclick`→open issue; `next.config.ts` adds its no-cache/Service-Worker-Allowed header. `lib/fcm.ts` (client) isSupported-gated `enableNotifications` (permission→getToken({vapidKey,serviceWorkerRegistration})→own `users/{uid}.fcmTokens` arrayUnion)+`listenForeground`. UI: `Toast`(provider/useToast, reused C9/C10)+`FcmForeground`(global onMessage→toast)+`NotificationOptIn`(reporter-only, states default/on/denied/unsupported)+`BreachBanner`(client-computed; sweep=C10), wired into `IssueDetail`+`layout`. Intake: notify seed reporter on a dedup **link** (post-commit). Timeline/SLA/StatusChip already live via onSnapshot — **zero change** to render a status_change row.
...
### C7 — Track: status machine + FCM + timeline — COMPLETE ✅ (local 21/21 + LIVE 4/4 + visual) · on-device push display = founder headed check — opens Phase: Depth
