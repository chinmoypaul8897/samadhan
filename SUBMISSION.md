...
- **Firebase** — Auth (anonymous — the whole product runs without forcing sign-up; a phone-OTP upgrade to save reports across devices is on the roadmap), Firestore (the full data model with live `onSnapshot` for the agent trace and timelines), Storage (citizen photos + before/after proof), Cloud Messaging (status push).
