...
- **Languages:** UI is English (full i18n is roadmap). **Vernacular is delivered where it matters — the filed complaint:** the agent auto-detects the citizen's language from their voice/text (Gemini transcription + language detection) and drafts the formal complaint in it. No manual language toggle (a header EN/हि pill misleads — it implies UI translation; and for a Bengaluru/Karnataka demo Kannada/English is the right vernacular, not Hindi). Superseded the earlier "English + Hindi toggle" plan — see progress.md.
